...
本文（moltenvk_design.md）定位为**总体架构概览**：项目定位、顶层结构、构建体系、分层架构全貌与设计取舍速查。各子系统的详细设计方案（类职责、关键流程、数据结构、实现策略）拆分到以下模块文档：

| 文档 | 覆盖范围 | 对应源码 |
|---|---|---|
| [module_api_entry.md](module_api_entry.md) | Vulkan C ABI 入口模板、proc-addr 分发与入口表、私有/弃用 API、Layer/Extension X-macro 注册 | `Vulkan/` · `Layers/` · `API/` · `Common/` |
| [module_core_objects.md](module_core_objects.md) | 对象基类体系（dispatchable handle）、MVKInstance/MVKPhysicalDevice/MVKDevice/MVKQueue、格式映射表 | `GPUObjects/`（Instance/Device/Queue/PixelFormats） |
| [module_commands.md](module_commands.md) | 命令对象与类型化对象池、录制-提交模型、MVKCommandEncoder 编码与脏状态跟踪 | `Commands/` |
| [module_resources_memory.md](module_resources_memory.md) | DeviceMemory/Buffer/Image/ImageView 内存模型、存储模式映射、描述符集→argument buffer | `GPUObjects/`（Buffer/Image/DeviceMemory/Descriptor） |
| [module_sync_wsi.md](module_sync_wsi.md) | Semaphore/Fence/Event 策略类层次、Timeline Semaphore、屏障落地策略、CAMetalLayer swapchain | `GPUObjects/`（Sync/Swapchain/Surface） |
| [module_shaders_pipeline.md](module_shaders_pipeline.md) | SPIR-V→MSL 转换数据结构与流程、MTLLibrary 三层缓存、管线创建、离线转换工具 | `MoltenVKShaderConverter/` + `MVKShaderModule/MVKPipeline` |

阅读建议：先读本文掌握全貌，再按子系统进入对应模块文档；模块文档间以 `module_xxx.md §n` 互相引用。
